        ...
        {hasVariant && (
          <thead>
            <tr>
              ...
              {hasVariant && <th className="view_tablefontsize large-screen-only">Vehicle 3</th>}
            </tr>
          </thead>
        )}
        <tbody>
          {tableData?.map((row, index) => (
            <tr key={index}>
              ...
              {row?.option_two && (
                <td className="fw-400 view_tablefontsize large-screen-only table_rowstext">{row.option_two}</td>
              )}
              {row?.option_three && (
